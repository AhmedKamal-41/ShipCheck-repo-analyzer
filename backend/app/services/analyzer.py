...
class CheckResult:
    ...
    evidence: dict[str, str]  # file, snippet
    recommendation: str
    points: int = 0
...
def analyze(fetch_result: dict[str, Any]) -> ReportResult:
    ...
    total = run_score + eng_score + sec_score + doc_score
    max_possible = 10 * (len(run_checks) + len(eng_checks) + len(sec_checks) + len(doc_checks))
