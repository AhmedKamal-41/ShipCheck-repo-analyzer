...
test.describe('Report Page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Mock API call
    await page.route(`**/api/reports/${doneReport.id}`, async (route) => {
  ...
  test('should display score summary', async ({ page }) => {
    await expect(page.getByText(/85/i).or(page.getByText(/score/i))).toBeVisible({ timeout: 5000 });
  });

  test('should display sections with checks', async ({ page }) => {
    // Check for section names
    await expect(page.getByText(/runability/i).or(page.getByText(/engineering/i))).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should copy share link', async ({ page, context }) => {
    ...
    if (await copyButton.isVisible()) {
      ...
      // Check clipboard (may need to verify UI feedback instead)
      const copiedText = await page.evaluate(() => navigator.clipboard.readText()).catch(() => null);
      // Or check for "Copied!" message
