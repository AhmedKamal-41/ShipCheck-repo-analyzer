      ...
      <div className="mt-5 space-y-3">
        {isInterviewPack ? (
          filteredQuestions.length === 0 ? (
            <p className="text-sm text-slate-500">No questions match the filter.</p>
          ) : (
            <ol className="list-inside list-decimal space-y-3 text-slate-700 leading-relaxed">
              {filteredQuestions.map((q, i) => (
                <li key={i} className="rounded-lg bg-slate-50/50 p-3 text-sm">
                  {q}
                </li>
              ))}
            </ol>
          ...
          <p className="text-sm text-slate-500">No checks match the filter.</p>
        ) : (
